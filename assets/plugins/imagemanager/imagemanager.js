$(document).ready(function(){
	...
	$('#imagemanager-save').click(function(){
		...
		$.ajax({
			...
			success     : function(ret){
							$('#modal-crop').modal('hide');
							var id_modal = $('#modal-crop').prev().prev().attr('data-image-id');
...
$(document).ready(function(){
	$(document).on('mousewheel DOMMouseScroll','#crop_container', function(e) {
	    ...
	    if ( delta ) {
	        ...
	        var step = $('#zoomSlider').slider("option", "step");
	            step *= delta < 0 ? 1 : -1;
	        $('#zoomSlider').slider("value", $('#zoomSlider').slider("value") + step);
	    }
	});
