...
class Event_category extends CI_Controller {
	...
	function index(){
		...
		$data['list_parent_category'] = selectlist2(array('table'=>'event_category','title'=>'All Category','where'=> 'is_hide = 0 and id_lang = '.$lang.' and is_delete = 0 and id_parent_category = 0 '));
		render('apps/event_category/index',$data,'apps');
	}
	public function add($id=''){
		if($id){
			// $data = $this->EventCategoryModel->findById($id);
			$datas = $this->EventCategoryModel->selectData($id);
            if(!$datas){
				die('404');
			}
		...
		foreach ($data['list_lang'] as $key => $value){
			...
			$data['list_lang'][$key]['list_parent_category']	= selectlist2(array('table'=>'event_category','title'=>'All Category','where'=> 'is_hide = 0 and id_lang = 1 and is_delete = 0 and id_parent_category = 0 ','selected'=>$datas[0]['id_parent_category']));
		}

		$data['list_lang2'] 	= $data['list_lang'];
		render('apps/event_category/add',$data,'apps');
	}
	function records(){
		...
		render('apps/event_category/records',$data,'blank');
	}
	function proses($idedit=''){
		...
		foreach ($post['name'] as $key => $value) {
			if(!$idedit){
				...
				$unik 					= $this->EventCategoryModel->findBy($where);
			...
			$data_save['id_parent_category']= $post['id_parent_category'][$key];
			...
			if($idedit && $post['img'][$key]){
				...
				$datas 				= $this->EventCategoryModel->selectData($idedit);
			...
			if($idedit){
				if($key==0){
					...
					$iddata 			= $this->EventCategoryModel->update($data_save,$idedit);
				}else{
					auth_update();
					$ret['message'] 	= 'Update Success';
					$act				= "Update News Category";
					$iddata 			= $this->EventCategoryModel->updateKedua($data_save,$idedit);
				}					
			}else{
				auth_insert();
				$ret['message'] 	= 'Insert Success';
				$act				= "Insert News Category";
				$iddata 			= $this->EventCategoryModel->insert($data_save);
			}
	...
	function del(){
		...
		$this->EventCategoryModel->delete($id);
		$this->EventCategoryModel->delete2($id);
		$this->db->trans_complete();
	}
	function record_select_page(){
		...
		render('apps/event_category/record_select_page',$data,'blank');
	}
