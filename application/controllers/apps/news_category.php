...
class News_category extends CI_Controller {
	...
	function index(){
		...
		$data['list_event_category'] = selectNewsCat(array('table'=>'news_category','where'=> array('is_hide'=>0,'is_delete'=>0,'id_lang'=>$id_lang),'title'=>'Select Event Category'));
	...
	public function add($id=''){
		...
		foreach ($data['list_lang'] as $key => $value){
			...
			$data['list_lang'][$key]['id_parent_category']  = $datas[$key]['id_parent_category'];
			...
			$data['list_lang'][$key]['hide_checked']        = ($datas[$key]['is_hide'] == 1) ? 'checked="checked"' : '';

			$data['list_lang'][$key]['list_news_category']  = selectNewsCat(array('table'=>'news_category','where'=> array('is_hide'=>0,'is_delete'=>0,'id_lang'=>$value['lang_id']),'title'=>'Select Parent Category','selected'=>$datas[$key]['id_parent_category']));
	...
	function proses($idedit=''){
		...
		foreach ($post['name'] as $key => $value) {
			...
			if($key==0){
				$id_parent_category = $post['id_parent_category'][0];
			} else{
				$id_parent_category 	= db_get_one('news_category','id',array('id_parent_lang'=>$post['id_parent_category'][0],'id_lang'=>$post['id_lang'][$key]));
			}
			...
			$data_save['id_parent_category'] = $id_parent_category;
