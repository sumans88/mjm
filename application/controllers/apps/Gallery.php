...
class Gallery extends CI_Controller {
	...
	public function add($id=''){
		...
		foreach ($data['list_lang'] as $key => $value){
			...
			$data['list_lang'][$key]['description']           = htmlspecialchars_decode(quote_form($datas[$key]['description']));
			$data['list_lang'][$key]['uri_path']              = $datas[$key]['uri_path'];
			$data['list_lang'][$key]['content']               =  htmlspecialchars_decode(urldecode($datas[$key]['content']));
	...
	function proses($idedit=''){
		...
		foreach ($post['name'] as $key => $value){
			...
			$data_save['id_gallery_category']	= 1;
	...
	function prosesgallery($id_gallery,$idedit=''){
	 ...
	 function imagemanager(){
		...
		if($post['id']) { //delete image
			$this->galleryImagesModel->delete($post['id'], array('user_id_modify'=>id_user(),'modify_date'=>date('Y-m-d H:i:s')));
		}
		$total_records = $this->galleryImagesModel->getTotal("(user_id_create = ".id_user() .") and name LIKE '%".$post['searchPicture']."%' and create_date LIKE '%".$tglsearch."%'");
		$per_page = 12;
		$data['pages_server'] = ceil($total_records/$per_page);
		$data['load'] = base_url().'apps/gallery/imagemanager';
		...
		$data['list_data'] = $this->galleryImagesModel->getAll("(user_id_create = ".id_user() ." ) and name LIKE '%".$post['searchPicture']."%' and create_date LIKE '%".$tglsearch."%'", $per_page, $offset);
		render('apps/filemanager_server',$data,'blank');
