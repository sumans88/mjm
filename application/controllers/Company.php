...
class Member extends CI_Controller {
	...
	function index(){
		...
		if($user_sess_data){
			...
			// $data                    = $this->pagesmodel->pages_content('members');
			// $data['content']         = render("member/index",$data,'blank'); // pake banner top
			...
			// return render('pages_blank',$data);
		}
	...
	function profile(){
		...
		if($user_sess_data){
			...
			$temp['member_name'] 		= $data_member['prefix_name'].' '.$data_member['firstname'].' '.$data_member['lastname'];
			$temp['member_job']         = $data_member['job'];
			$temp['member_email']       = $data_member['email'];
			$temp['member_linkedin_id'] = $data_member['linkedin_id'];

			//company
			$data_company = $this->company_model->findById($user_sess_data['company_id']);
			...
			//membership
			unset($temp);
			$temp['membership_code']       = '';
			$temp['membership_date']       = '';
			$temp['membership_registered'] = '';
			$temp['membership_expired']    = '';
			$temp['membership_lastvisit']  = '';
			$data['membership'][]  = $temp;
			
			$data['is_membership'] = ($data['membership'][0]['membership_code']) ? '' :'hide';
			$data['not_membership'] = ($data['membership'][0]['membership_code']) ? 'hide' :'';
	...
	function payment_confirmation_proses(){		
		...
		if (!$data_invoice) {
			$ret['error'] = 1;
			$ret['msg'] = 'invoice not found for this account';
			// $ret['parsley'] = 'invoice_number';
			// $ret['parsley_message'] = 'invoice not found for this account';
		...
		$invoice_save['invoice_number']          = $post['invoice_number'];
		...
		//sent email to admin 
			...
			$email_admin['name']           = $user_sess_data['member_namadepan'];
	...
	function register($category){
		...
		switch ($category) {
			...
			case 'individual': 
				$data['id_category'] = 2 ;
				$data['is_required']  = "required data-parsley-type='email' ";
				$data['is_hide']  = '';
			break;
			default: redirect('member'); break;
		}
	...
	function register_proses(){
		$post                       = $this->input->post();
		if ($post['member']['member_category_id'] == 1) {
			$where['email'] = $post['company']['email'];
			$check_company  = $this->company_model->findBy($where,1);
		}else{
		...
		if ($check_user || $check_company) {
			$ret['error'] = 1;
			$ret['modalname'] = 'myModalThanks';
			// $ret['msg'] = 'you had registered before';
			echo json_encode($ret);exit;
		}

		$password                   = generatePassword();
		$post['member']['email']    = ($post['member']['member_category_id'] == 1) ? $post['company']['email'] : $post['member']['email'];// company		
		...
		$this->member_model->update($update_member,$id_member);
		//send email to user

		$email_user['name']     = $post['member']['prefix_name'].' '.$post['member']['firstname'].' '.$post['member']['middle'].' '.$post['member']['lastname'];
		$email_user['category'] = db_get_one('auth_member_category','name',array('id'=>$post['member']['member_category_id']));
		$email_user['username'] = ( $post['member']['member_category_id'] == 1) ? $post['company']['email'] : $post['member']['email'];
		...
		$email_admin['name']     = $post['member']['prefix_name'].' '.$post['member']['firstname'].' '.$post['member']['middle'].' '.$post['member']['lastname'];

		$company_detail = $this->company_model->findById($id_company);
		$email_admin['company'] = $company_detail['name_in'].' / '.$company_detail['name_out'];
