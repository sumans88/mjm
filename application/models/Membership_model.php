...
class membership_model extends  CI_Model{
	var $table = 'membership_information';
	var $tableAs = 'membership_information a';
	...
	function findById($id){
		$where['a.id'] = $id;
		$this->db->select("a.*");
		// $this->db->join('company b',"b.member_id_create		 = a.id",'left');
	...
	function findViewBy($where,$is_single_row=0){
		...
		if($is_single_row==1){
			...
			return 	$this->db->get_where($this->tableview,$where)->result_array();
		}
	} 
