...
class Banner_Model extends  CI_Model{
	...
	function records($where=array(),$isTotal=0){
		...
		$alias['search_banner_title'] = 'a.banner_title';
		$alias['search_banner_title_e'] = 'x.banner_title';
		$alias['search_banner_position'] = 'e.id';
		// $ttl_row = $this->db->get($this->tableAs)->num_rows();

	 	query_grid($alias,$isTotal);
		$this->db->select("a.*,c.name as status,d.username, x.banner_title as banner_title_e, e.name as banner_position");
		...
		$this->db->join('banner_position e',"e.id = a.id_banner_position",'left');
		$this->db->join('banner x',"x.id_parent_lang = a.id",'left');
